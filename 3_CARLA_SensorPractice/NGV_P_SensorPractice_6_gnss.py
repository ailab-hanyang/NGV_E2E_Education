#!/usr/bin/env python3
#======================================================================#
# Module:      NGV_SensorPractice_6_gnss.py
# Description: Sensor practice - GNSS terminal and map output
#
...
# Map style
# ============================================================
MAP_BG_COLOR = (0, 0, 0)         # 검정 배경
MAP_WP_COLOR = (255, 255, 255)   # 차선 점을 흰색
MAP_WP_RADIUS = 3                # 더 크게 (2~4 추천)
...
def prepare_map_cache(world):
    ...
    usable_w = max(WINDOW_W - 2 * MAP_MARGIN_PX, 1)
    usable_h = max(WINDOW_H - 2 * MAP_MARGIN_PX, 1)
...
def main():
    ...
    # map cache + base surface
    map_cache = None
    base_map_surface = None

    # pygame
    screen = pygame.display.set_mode((WINDOW_W, WINDOW_H), pygame.HWSURFACE | pygame.DOUBLEBUF)
    pygame.display.set_caption("NGV | GNSS(+MAP) | C:Cam | V:MapTop | Q:Manual | R:Reverse | ESC Quit")
    ...
    try:
        ...
        map_cache = prepare_map_cache(world)
        base_map_surface = pygame.Surface((WINDOW_W, WINDOW_H))
        base_map_surface.fill(MAP_BG_COLOR)

        if map_cache is not None:
            print("[MAP] waypoints:", len(map_cache["wps"]))

            for wp in map_cache["wps"][::int(MAP_DRAW_STRIDE)]:
                loc = wp.transform.location
                px, py = world_to_screen(map_cache, loc.x, loc.y)
                if 0 <= px < WINDOW_W and 0 <= py < WINDOW_H:
                    pygame.draw.circle(base_map_surface, MAP_WP_COLOR, (px, py), MAP_WP_RADIUS)
        else:
        ...
        while True:
            ...
            now = time.time()
            if last_gnss is not None and (now - last_print_t) >= float(TERM_PRINT_INTERVAL_SEC):
                last_print_t = now
                print(f"[GNSS] lat={last_gnss.latitude:.6f}, lon={last_gnss.longitude:.6f}, alt={last_gnss.altitude:.2f}")

            screen.blit(base_map_surface, (0, 0))

            if map_cache is not None and is_alive(ego_vehicle):
                loc = ego_vehicle.get_transform().location
                ex, ey = world_to_screen(map_cache, loc.x, loc.y)

                pygame.draw.circle(screen, (255, 0, 0), (ex, ey), 6)

                fwd = ego_vehicle.get_transform().get_forward_vector()
                hx, hy = world_to_screen(map_cache, loc.x + fwd.x * 8.0, loc.y + fwd.y * 8.0)
                pygame.draw.line(screen, (255, 0, 0), (ex, ey), (hx, hy), 2)
            ...
            if last_gnss is not None:
                hud.append(f"lat={last_gnss.latitude:.6f}  lon={last_gnss.longitude:.6f}  alt={last_gnss.altitude:.2f}m")
